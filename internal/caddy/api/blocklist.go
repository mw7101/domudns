...
// BlocklistStore abstracts blocklist persistence.
type BlocklistStore interface {
	ListBlocklistURLs(ctx context.Context) ([]store.BlocklistURL, error)
	AddBlocklistURL(ctx context.Context, url string, enabled bool) (*store.BlocklistURL, error)
	...
	ListBlocklistPatterns(ctx context.Context) ([]store.BlocklistPattern, error)
	AddBlocklistPattern(ctx context.Context, pattern string, patternType string) (*store.BlocklistPattern, error)
...
func NewBlocklistHandler(store BlocklistStore, cfg *config.Config, corednsReload CoreDNSReloader) *BlocklistHandler {
	h := &BlocklistHandler{
		store:         store,
		cfg:           cfg,
